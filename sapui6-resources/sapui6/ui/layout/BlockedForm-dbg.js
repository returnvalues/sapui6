jQuery.sap.declare("sapui6.ui.layout.BlockedForm");
jQuery.sap.require("sap.ui.core.Control");

sap.ui.core.Control.extend("sapui6.ui.layout.BlockedForm", { 
    metadata : {     
    ...
    onBeforeRendering : function(){
  	...
  	renderer : function(oRm, oControl){
		...
		for(var i=0 ; i<length ; i++){
			...
			if(item["colspan"] == undefined) layoutCnt ++;
			else layoutCnt += parseInt(item["colspan"]);
			
			if(layoutCnt%columns == 1){
				oRm.write("<tr>");
			}
			...
			if(item["colspan"] != undefined){
				oRm.writeAttribute("colspan", parseInt(item["colspan"]));
			}else if(i == length-1) {
				oRm.writeAttribute("colspan", columns - (layoutCnt-1));
			}

			if(item["align"] != undefined) {
				oRm.addStyle("text-align", item["align"]);
				oRm.writeStyles();
			}
			...
			oRm.write("<span>");
				item["element"].forEach(function(ele){
					oRm.write("<span style='margin-right:5px;'>");
...
sapui6.ui.layout.BlockedForm.prototype.resizeLayout = function(widthRatio){
...
sapui6.ui.layout.BlockedForm.prototype.set = function(oSettings){
